    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        // 1. Archivos estáticos: Usa rutas directas en lugar de comodines complejos
                        .requestMatchers("/", "/index.html", "/favicon.ico", "/favico.png").permitAll()
                        .requestMatchers("/assets/**", "/static/**").permitAll()

                        // 2. Rutas de la API (Aseguramos que /api/ esté cubierto correctamente)
                        .requestMatchers("/auth/**", "/api/auth/**").permitAll()
                        .requestMatchers("/catalogos/**", "/api/catalogos/**").permitAll()
                        .requestMatchers("/public/**", "/api/public/**").permitAll()

                        // 3. Error page (Importante para que no entre en bucle infinito)
